...
exports.update = async (req, res) => {
  ...
  try {
    if (title === "") req.body.title = "Sans Nom";
    if (description === "") req.body.description = "Ajouter une description";
    const currentBoard = await Board.findById(boardId);
    if (!currentBoard) return res.status(404).json("Tableau non trouvé");
